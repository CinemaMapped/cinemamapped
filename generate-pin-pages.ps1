...
foreach ($pin in $data) {
    ...
    $html += "    var icon = L.divIcon({ className:'', html:'<div style=""width:14px;height:14px;border-radius:50%;background:#c9a84c;border:2px solid #fff;""></div>', iconSize:[14,14], iconAnchor:[7,7] });`n"
    $html += "    L.marker([$lat, $lng], { icon:icon, interactive:false }).addTo(m);`n"
