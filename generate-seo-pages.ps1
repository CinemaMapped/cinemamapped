...
foreach ($title in $allTitles) {
    ...
    $schemaJson = "  {`n    `"@context`": `"https://schema.org`",`n    `"@type`": `"ItemList`",`n    `"name`": `"$titleJson - Real WWII Locations`",`n    `"description`": `"Real WWII historical locations where $titleJson is set, tracked on CinemaMapped.`",`n    `"url`": `"$canonUrl`",`n    `"numberOfItems`": $pinCount`n  }"
    ...
    $html += "  <title>Where Was $titleHtml Set? Real WWII Locations | CinemaMapped</title>`n"
    ...
    $html += "  <meta property=`"og:title`" content=`"Where Was $titleHtml Set? | CinemaMapped`">`n"
    ...
    $html += "    <h1 class=`"pin-page-h1`">$titleHtml &mdash; Where the Story Was Set</h1>`n"
    $html += "    <p class=`"pin-page-title-line`">$pinCount real historical $locWord &middot; $filmCountriesHtml</p>`n"
    $html += "  </div>`n`n"
...
$filmsIndex += "  <meta property=`"og:title`" content=`"All WWII Films and Series | CinemaMapped`">`n"
$filmsIndex += "  <meta property=`"og:description`" content=`"Browse $totalTitles WWII films and series with $totalPins pinned real historical locations. Find where Band of Brothers, Saving Private Ryan, Schindler's List and more are set.`">`n"
$filmsIndex += "  <meta property=`"og:type`" content=`"website`">`n"
$filmsIndex += "  <meta property=`"og:url`" content=`"https://cinemamapped.com/films`">`n"
$filmsIndex += "  <meta property=`"og:image`" content=`"https://cinemamapped.com/og-image.svg`">`n"
$filmsIndex += "  <script type=`"application/ld+json`">`n  {`n    `"@context`": `"https://schema.org`",`n    `"@type`": `"CollectionPage`",`n    `"name`": `"WWII Films and Series | CinemaMapped`",`n    `"description`": `"A curated collection of $totalTitles WWII films and series with $totalPins real historical locations mapped across Europe, Asia, the Pacific, and North Africa.`",`n    `"url`": `"https://cinemamapped.com/films`",`n    `"numberOfItems`": $totalTitles`n  }`n  </script>`n"
...
$filmsIndex += "  <style>.chip[data-filter-theatre='Western Front']{color:#c9a84c;border-color:rgba(201,168,76,.25)}.chip[data-filter-theatre='Eastern Front']{color:#c94c4c;border-color:rgba(201,76,76,.25)}.chip[data-filter-theatre='Pacific']{color:#4c9bc9;border-color:rgba(76,155,201,.25)}.chip[data-filter-theatre='North Africa']{color:#c98b4c;border-color:rgba(201,139,76,.25)}.chip[data-filter-theatre='Atlantic']{color:#4c6ec9;border-color:rgba(76,110,201,.25)}.chip[data-filter-theatre='Mediterranean']{color:#4cc9b0;border-color:rgba(76,201,176,.25)}.chip.active[data-filter-theatre='Western Front']{border-color:#c9a84c;background:rgba(201,168,76,.12)}.chip.active[data-filter-theatre='Eastern Front']{border-color:#c94c4c;background:rgba(201,76,76,.12)}.chip.active[data-filter-theatre='Pacific']{border-color:#4c9bc9;background:rgba(76,155,201,.12)}.chip.active[data-filter-theatre='North Africa']{border-color:#c98b4c;background:rgba(201,139,76,.12)}.chip.active[data-filter-theatre='Atlantic']{border-color:#4c6ec9;background:rgba(76,110,201,.12)}.chip.active[data-filter-theatre='Mediterranean']{border-color:#4cc9b0;background:rgba(76,201,176,.12)}</style>`n"
...
$filmsIndex += "  </div>`n`n"
$filmsIndex += "  <div class=`"index-search-wrap`">`n"
$filmsIndex += "    <div class=`"index-search-container`">`n"
$filmsIndex += "      <svg class=`"index-search-icon`" viewBox=`"0 0 16 16`" width=`"14`" height=`"14`" fill=`"none`" stroke=`"currentColor`" stroke-width=`"1.8`"><circle cx=`"6.5`" cy=`"6.5`" r=`"5`"/><line x1=`"10.5`" y1=`"10.5`" x2=`"14`" y2=`"14`"/></svg>`n"
$filmsIndex += "      <input type=`"search`" id=`"film-search`" class=`"index-search`" placeholder=`"Search by title, location or country...`" autocomplete=`"off`">`n"
$filmsIndex += "    </div>`n"
$filmsIndex += "  </div>`n"
$filmsIndex += "  <div class=`"index-filter-bar`">`n"
$filmsIndex += "    <div class=`"index-filter-group`">`n"
$filmsIndex += "      <span class=`"index-filter-group-label`">Type</span>`n"
...
$filmsIndex += "    <div class=`"index-filter-group`">`n"
$filmsIndex += "      <span class=`"index-filter-group-label`">Theatre</span>`n"
$filmsIndex += "      <button class=`"chip active`" data-filter-theatre=`"all`">All</button>`n"
...
$filmsIndex += "      <button class=`"chip`" data-filter-theatre=`"North Africa`">North Africa</button>`n"
...
$filmsIndex += "    </div>`n"
$filmsIndex += "    <div class=`"index-filter-sep`"></div>`n"
$filmsIndex += "    <select id=`"film-sort`" class=`"index-sort`">`n"
$filmsIndex += "      <option value=`"az`">A &ndash; Z</option>`n"
$filmsIndex += "      <option value=`"locs`">Most locations</option>`n"
$filmsIndex += "    </select>`n"
$filmsIndex += "    <span class=`"filter-count-label`" id=`"filter-count`">$totalTitles titles</span>`n"
$filmsIndex += "  </div>`n"
$filmsIndex += "  <div class=`"index-grid`" id=`"film-grid`">`n"
...
$filmsIndex += "  var at='all',ath='all',sq='',srt='az';`n"
$filmsIndex += "  var grid=document.getElementById('film-grid');`n"
$filmsIndex += "  var allCards=Array.from(document.querySelectorAll('#film-grid .index-card[data-type]'));`n"
$filmsIndex += "  function run(){`n"
$filmsIndex += "    var n=0;`n"
$filmsIndex += "    allCards.forEach(function(c){`n"
$filmsIndex += "      var title=c.querySelector('.index-card-title');`n"
$filmsIndex += "      var t=title?title.textContent.toLowerCase():'';`n"
$filmsIndex += "      var th=c.dataset.theatre?c.dataset.theatre.toLowerCase():'';`n"
$filmsIndex += "      var matchSq=sq===''||t.indexOf(sq)!==-1||th.indexOf(sq)!==-1;`n"
$filmsIndex += "      var ok=matchSq&&(at==='all'||c.dataset.type===at)&&(ath==='all'||c.dataset.theatre===ath);`n"
...
$filmsIndex += "    var sorted=allCards.filter(function(c){return c.style.display!=='none';});`n"
$filmsIndex += "    if(srt==='locs')sorted.sort(function(a,b){return parseInt(b.dataset.count||0)-parseInt(a.dataset.count||0);});`n"
$filmsIndex += "    sorted.forEach(function(c){grid.appendChild(c);});`n"
$filmsIndex += "  }`n"
$filmsIndex += "  var si=document.getElementById('film-search');`n"
$filmsIndex += "  if(si)si.addEventListener('input',function(){sq=this.value.toLowerCase().trim();run();});`n"
...
$filmsIndex += "    });`n"
$filmsIndex += "  });`n"
$filmsIndex += "  var so=document.getElementById('film-sort');`n"
$filmsIndex += "  if(so)so.addEventListener('change',function(){srt=this.value;run();});`n"
$filmsIndex += "})();`n"
$filmsIndex += "  </script>`n"
...
$countryCards = ($countriesSorted | ForEach-Object {
    ...
    "    <div class=`"index-card`" data-region=`"$cRegion`" data-locs=`"$cCount`" data-titles=`"$cFilms`" data-name=`"$cHtml`">`n      <a href=`"/countries/$cSlug`" class=`"index-card-link`">`n        <div class=`"index-card-title`">$cFlagHtml$cHtml</div>`n        <div class=`"index-card-meta`"><span>$cCount $cLocWord</span><span>&middot; $cFilms $cFilmWord</span></div>$cTopLine`n      </a>`n    </div>"
}) -join "`n"
...
$countriesIndex += "  <meta property=`"og:title`" content=`"WWII Film Locations by Country | CinemaMapped`">`n"
$countriesIndex += "  <meta property=`"og:description`" content=`"Explore $totalCountries countries featured in WWII cinema. Find real historical locations for every front - Western Europe, Eastern Front, Pacific, North Africa and more.`">`n"
$countriesIndex += "  <meta property=`"og:type`" content=`"website`">`n"
$countriesIndex += "  <meta property=`"og:url`" content=`"https://cinemamapped.com/countries`">`n"
$countriesIndex += "  <meta property=`"og:image`" content=`"https://cinemamapped.com/og-image.svg`">`n"
$countriesIndex += "  <script type=`"application/ld+json`">`n  {`n    `"@context`": `"https://schema.org`",`n    `"@type`": `"CollectionPage`",`n    `"name`": `"WWII Film Locations by Country | CinemaMapped`",`n    `"description`": `"$totalCountries countries featured across $totalTitles WWII films and series. Every pin marks a real historical location where the story was set.`",`n    `"url`": `"https://cinemamapped.com/countries`",`n    `"numberOfItems`": $totalCountries`n  }`n  </script>`n"
...
$countriesIndex += "  <style>.chip[data-filter-theatre='Western Front']{color:#c9a84c;border-color:rgba(201,168,76,.25)}.chip[data-filter-theatre='Eastern Front']{color:#c94c4c;border-color:rgba(201,76,76,.25)}.chip[data-filter-theatre='Pacific']{color:#4c9bc9;border-color:rgba(76,155,201,.25)}.chip[data-filter-theatre='North Africa']{color:#c98b4c;border-color:rgba(201,139,76,.25)}.chip[data-filter-theatre='Atlantic']{color:#4c6ec9;border-color:rgba(76,110,201,.25)}.chip[data-filter-theatre='Mediterranean']{color:#4cc9b0;border-color:rgba(76,201,176,.25)}.chip.active[data-filter-theatre='Western Front']{border-color:#c9a84c;background:rgba(201,168,76,.12)}.chip.active[data-filter-theatre='Eastern Front']{border-color:#c94c4c;background:rgba(201,76,76,.12)}.chip.active[data-filter-theatre='Pacific']{border-color:#4c9bc9;background:rgba(76,155,201,.12)}.chip.active[data-filter-theatre='North Africa']{border-color:#c98b4c;background:rgba(201,139,76,.12)}.chip.active[data-filter-theatre='Atlantic']{border-color:#4c6ec9;background:rgba(76,110,201,.12)}.chip.active[data-filter-theatre='Mediterranean']{border-color:#4cc9b0;background:rgba(76,201,176,.12)}</style>`n"
...
$countriesIndex += "    <div class=`"index-filter-group`">`n"
$countriesIndex += "      <span class=`"index-filter-group-label`">Region</span>`n"
...
$countriesIndex += "      <button class=`"chip`" data-filter-region=`"Africa`">North Africa</button>`n"
$countriesIndex += "      <button class=`"chip`" data-filter-region=`"Americas`">Americas</button>`n"
$countriesIndex += "    </div>`n"
$countriesIndex += "    <div class=`"index-filter-sep`"></div>`n"
$countriesIndex += "    <select id=`"country-sort`" class=`"index-sort`">`n"
$countriesIndex += "      <option value=`"locs`">Most locations</option>`n"
$countriesIndex += "      <option value=`"titles`">Most titles</option>`n"
$countriesIndex += "      <option value=`"az`">A &ndash; Z</option>`n"
$countriesIndex += "    </select>`n"
$countriesIndex += "    <span class=`"filter-count-label`" id=`"country-filter-count`">$totalCountries countries</span>`n"
$countriesIndex += "  </div>`n"
$countriesIndex += "  <div class=`"index-grid`" id=`"country-grid`">`n"
...
$countriesIndex += "  var ar='all',srt='locs';`n"
$countriesIndex += "  var grid=document.getElementById('country-grid');`n"
$countriesIndex += "  var allCards=Array.from(document.querySelectorAll('#country-grid .index-card[data-region]'));`n"
$countriesIndex += "  function run(){`n"
$countriesIndex += "    var n=0;`n"
$countriesIndex += "    allCards.forEach(function(c){`n"
...
$countriesIndex += "    var sorted=allCards.filter(function(c){return c.style.display!=='none';});`n"
$countriesIndex += "    if(srt==='locs')sorted.sort(function(a,b){return parseInt(b.dataset.locs||0)-parseInt(a.dataset.locs||0);});`n"
$countriesIndex += "    else if(srt==='titles')sorted.sort(function(a,b){return parseInt(b.dataset.titles||0)-parseInt(a.dataset.titles||0);});`n"
$countriesIndex += "    else sorted.sort(function(a,b){return(a.dataset.name||'').localeCompare(b.dataset.name||'');});`n"
$countriesIndex += "    sorted.forEach(function(c){grid.appendChild(c);});`n"
$countriesIndex += "  }`n"
...
$countriesIndex += "    });`n"
$countriesIndex += "  });`n"
$countriesIndex += "  var so=document.getElementById('country-sort');`n"
$countriesIndex += "  if(so)so.addEventListener('change',function(){srt=this.value;run();});`n"
$countriesIndex += "})();`n"
$countriesIndex += "  </script>`n"
